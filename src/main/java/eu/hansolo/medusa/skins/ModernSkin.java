...
//        sectionListener      = c -> handleEvents("RESIZE");
    ...
    private void initGraphics() {
        ...
        valueText = new Text(formatNumber(gauge.getLocale(), null/*gauge.getFormatString()*/, gauge.getDecimals(), gauge.getCurrentValue()));
...
//            sectionsVisible = gauge.getSectionsVisible();
    ...
    public void handleMouseEvent(final MouseEvent EVENT) {
        ...
        if (MouseEvent.MOUSE_PRESSED.equals(TYPE)) {
            ...
            valueText.setLayoutY(size * 0.501);
            subTitleText.setLayoutY(size * 0.3525);
            unitText.setLayoutY(size * 0.6675);
            ...
            valueText.setLayoutY(size * 0.5);
            subTitleText.setLayoutY(size * 0.35);
            unitText.setLayoutY(size * 0.67);
        }
    }
...
//        gauge.getSections().removeListener(sectionListener);
    ...
    private void rotateNeedle(final double VALUE) {
        ...
        needleRotate.setAngle(Helper.clamp(180 - START_ANGLE, 180 - START_ANGLE + ANGLE_RANGE, targetAngle));
        valueText.setText(formatNumber(gauge.getLocale(), null/*gauge.getFormatString()*/, gauge.getDecimals(), VALUE));

        if (valueText.getText().length() != valueLength) // Only needed in this case
            resizeText();

/* Commented as doesn't seem necessary
        placeTextVerticaly();
*/
    ...
    private void highlightValue( final GraphicsContext CTX, final double CURRENT_VALUE ) {
        ...
        valueText.setText(formatNumber(gauge.getLocale(), null/*gauge.getFormatString()*/, gauge.getDecimals(), CURRENT_VALUE));
...
*/
        ...
        for (double angle = 0 ; Double.compare(-ANGLE_RANGE - tmpAngleStep, angle) < 0 ; angle -= tmpAngleStep) {
            ...
            if (Double.compare(counterBD.remainder(majorTickSpaceBD).doubleValue(), 0.0) == 0) {
                ...
                if (tickLabelsVisible) {
                    ...
                    CTX.fillText(Helper.formatNumber(locale, null, decimals, counter) /*String.format(locale, "%." + decimals + "f", counter)*/, 0, 0);
                    CTX.restore();
                }
            } else if (mediumTickMarksVisible &&
                       Double.compare(minorTickSpaceBD.remainder(mediumCheck2).doubleValue(), 0.0) != 0.0 &&
    ...
    private void resizeText() {
        ...
        if (titleText.getText().length() > titleLength) {
            if (titleWidth > maxWidth) { Helper.adjustTextSize(titleText, maxWidth, size * 0.062); }
        }
        titleText.setLayoutX((size - titleWidth) * 0.5);
    ...
    private void placeTextVerticaly() {
        ...
        titleText.setLayoutY(size * 0.83);
