...
public class Gauge extends Control {
    ...
    // Update events
    private              List<UpdateEvent>      updateEventQueue = new ArrayList<>(); // LinkedBlockingQueue<>();
    private List<UpdateEventListener>            listenerList     = new /*CopyOnWrite*/ArrayList<>();
...
*/
    ...
    private void setupBinding() {
        showing = Bindings.createBooleanBinding(() -> {
            if (getScene() != null && getScene().getWindow() != null) {
                if (getScene().getWindow().isShowing()) {
                    while(!updateEventQueue/*.peek() != null*/.isEmpty()) {
                        UpdateEvent event = updateEventQueue.remove(0); //updateEventQueue.poll();
        ...
        showing.addListener((o, ov, nv) -> {
            if (nv) {
                while(!updateEventQueue/*.peek() != null*/.isEmpty()) {
                    UpdateEvent event = updateEventQueue.remove(0); // .poll();
                    for (UpdateEventListener listener : listenerList) { listener.onUpdateEvent(event); }
                }
            }
        });
