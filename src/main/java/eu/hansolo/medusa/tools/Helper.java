            ...
            /*if (tickMarkSections.size() > 0) {
                ...
                for (int i = 0; i < listSize; i++) {
                    ...
                    if (Double.compare(section.getStart(), MIN_VALUE) < 0 && Double.compare(section.getStop(), MAX_VALUE) < 0) {
                        ...
                        sectionStartAngle = Gauge.ScaleDirection.CLOCKWISE == scaleDirection ? (section.getStart() - MIN_VALUE) * ANGLE_STEP : -(section.getStart() - MIN_VALUE) * ANGLE_STEP;
                    }
                    double sectionAngleExtend;
                    if (Double.compare(section.getStop(), MAX_VALUE) > 0) {
                        sectionAngleExtend = Gauge.ScaleDirection.CLOCKWISE == scaleDirection ? (MAX_VALUE - section.getStart()) * ANGLE_STEP : -(MAX_VALUE - section.getStart()) * ANGLE_STEP;
                    } else {
                    ...
                    switch (tickMarkType) {
                        ...
                        case TICK_LABEL:
                            if (majorTickMarksVisible) {
                                ...
                                CTX.fillText(formatNumber(locale, tickLabelFormatString, 0, counter) /*String.format(locale, tickLabelFormatString, counter)*/, 0, 0);
                                CTX.restore();
                            }
                            break;
                        case LINE:
